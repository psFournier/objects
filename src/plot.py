...
df = pd.concat([pd.read_pickle('../log/cluster/1305/*.pkl')], ignore_index=True)
...
df1 = df1[(df1['--nstep'] == 1)]
df1 = df1[(df1['--dropout'] == 0.5)]
# df1 = df1[(df1['--l2reg'] == 0)]
# df1 = df1[(df1['--seed'] == 13)]
# df1 = df1[(df1['--nbObjectsTrain'] == 9)]
...
for j, (name, g) in enumerate(df1.groupby(p)):
    ...
    for i, valy in enumerate(y):
        ...
        ax2[xplot,yplot].plot(g['trainstep'], g[valy]['median'].rolling(1).mean(), label=name, c=colors[j])
        ax2[xplot,yplot].fill_between(g['trainstep'],
                                g[valy]['quant_inf'].rolling(1).mean(),
                                g[valy]['quant_sup'].rolling(1).mean(), alpha=0.25, linewidth=0, color=colors[j])
